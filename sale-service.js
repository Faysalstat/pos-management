...
exports.issueSaleOrder = async (req) => {
  ...
  try {
    orderIssueModel = {
      ...
      voucherNo: voucherNo,
      clientId: clientId,
      isWalkingCustomer: isWalkingCustomer || false,
    };
...
exports.returnSaleOrder = async (req) => {
  ...
  let voucherNo = tnxDate.getTime();
  let invoice = await SaleInvoice.findOne({ where: { id: payload.invoiceId } });
  if (invoice && invoice.isWalkingCustomer) {
    returnAcc = await Account.findOne({
      where: {
        accountType: ACCOUNT_TYPES.CUSTOMER_RETURN_GL,
        clientId: clientId,
      },
      include: GlAccount,
    });
      ...
      if (payload.returnType == "RETURN") {
        ...
        if (invoice && invoice.isWalkingCustomer) {
          cashInHandAccTnxModel = {
            transactionType: TransactionTypes.CREDIT,
            transactionReason: "CASH in Hand Credited",
            transactionCategory: TRANSACTION_CATEGORY.PRODUCT_SALE_RETURN,
            amount: payload.totalSellPrice,
            transactionDate: tnxDate,
            approvedBy: payload.approveBy,
          ...
          let cashInHandAccTnx = await transactionRepository.doTransaction(
            cashInHandAccTnxModel
          );
        }
